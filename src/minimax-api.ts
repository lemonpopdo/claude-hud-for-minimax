 ...
 *
 * @param modelName - Optional specific model name to query. If omitted, auto-detects.
 *
 ...
 */
export async function fetchMiniMaxQuota(modelName?: string | null): Promise<MiniMaxQuotaData | null> {
  ...
  try {
    ...
    let entry: ModelRemainEntry | undefined;
    let matchedModelName: string;

    if (modelName) {
      // User-specified model name: match exact name first, then prefix
      entry = entries.find(e => e.model_name === modelName)
        ?? entries.find(e => e.model_name.startsWith(modelName));
      matchedModelName = entry?.model_name ?? modelName;
    } else {
      // Auto-detect: prefer MiniMax-M prefix, fallback to first entry
      const currentModel = process.env.ANTHROPIC_MODEL ?? '';
      entry = entries.find(e =>
        currentModel.startsWith('MiniMax-M') && e.model_name.startsWith('MiniMax-M')
      ) ?? entries[0];
      matchedModelName = entry?.model_name ?? '';
    }
...
/**
 * Fetch the list of available model names from the MiniMax API.
 * Returns null on any failure.
 */
export async function fetchMiniMaxModels(): Promise<string[] | null> {
  const token = process.env.ANTHROPIC_AUTH_TOKEN;
  if (!token) {
    debug('No ANTHROPIC_AUTH_TOKEN found, skipping model list fetch');
    return null;
  }
  ...
  try {
    ...
    return entries.map(e => e.model_name);
  } catch (err) {
    if (err instanceof Error) {
      if (err.message.includes('TIMEOUT') || err.message.includes('killed')) {
        debug(`curl timed out after ${TIMEOUT_MS}ms`);
