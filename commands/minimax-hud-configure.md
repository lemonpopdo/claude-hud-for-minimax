...
**FIRST**: Use the Read tool to load `~/.claude/plugins/minimax-claude-hud/config.json` if it exists.
...
- options: **ONLY items that are ON in the chosen preset** (max 4)
  ...
  - "Agents status" - Shows subagent status
  - "Todo progress" - Shows todo progress
  - "Project name" - Project path display
  - "Git status" - Branch indicator
  - "Config counts" - Shows config file counts
  - "Token breakdown" - Shows in/cache token counts
  - "Output speed" - Shows output token speed
  - "Usage limits" - Shows 5h/7d usage
  - "Session duration" - Shows session timer
  - "Session name" - Shows session slug or custom title
...
**Note:** If preset has all items ON (Full), skip Q4. If all OFF (Minimal), skip Q3.
...
- options:
  - "Skip" - No custom line
  - "Enter custom text" - Ask user for their phrase

If user chooses "Enter custom text", use AskUserQuestion to get their text (max 80 chars). Save as `display.customLine` in config.
...
- multiSelect: false
- options:
  - "Keep current" - No layout/preset changes
  - "Switch to Expanded" - Split into semantic lines
  - "Switch to Compact" - Everything on one line
  - "Reset to Full" - Enable everything
  - "Reset to Essential" - Activity + git only

### Q5: Custom Line (optional)
- header: "Custom Line"
- question: "Update your custom phrase? (currently: '{current value or none}')"
- multiSelect: false
- options:
...
If user chooses "Enter custom text", use AskUserQuestion to get their text (max 80 chars).
...
2. Apply Turn Off selections
3. Apply Turn On selections
...
Write to `~/.claude/plugins/minimax-claude-hud/config.json`.
